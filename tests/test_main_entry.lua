...
local function fakeBrowser()
    ...
    return {
        ...
        showSourceMultiDelete = function()
            calls.multidelete = (calls.multidelete or 0) + 1
        end,
    }
end
...
-- 「批量删除源」必须在「管理漫画源」里（真机 30 个源逐个删不是办法）。
function tests.source_menu_has_multi_delete_and_calls_browser()
    local browser = fakeBrowser()
    local items = probeMenu(newInstance(browser)).sub_item_table_func()
    local mgmt
    for _, it in ipairs(items) do
        if it.text == "管理漫画源" then mgmt = it end
    end
    ...
    for _, it in ipairs(mgmt.sub_item_table) do
        if it.text == "批量删除源（多选）" then row = it end
    end
    assert(type(row and row.callback) == "function", "缺「批量删除源（多选）」")
    row.callback()
    assert_eq("转发到 browser", 1, browser.calls.multidelete)
    return true
end
...
function tests.settings_autoflush_flushes_and_reschedules()
    ...
    local sched = withAutoflushStubs(function() n = n + 1 end, function(s)
        ...
        assert_eq("节拍 120 秒（L9：写盘频率降为 1/4）", 120, s[1].delay)
        s[1].cb()
