...
--- L4（审查报告 §4）：HTTP Expires 按 GMT 解析——本机时区无关。
--- 旧实现 os.time(table) 按本地时区解释，UTC+8 上早死 8 小时。
function tests.expires_gmt_interpretation()
    local epoch_ms = Cookies.parseHttpDate("Fri, 01 Jan 2030 00:00:00 GMT")
    assert(epoch_ms ~= nil, "RFC1123 date must parse")
    -- 用测试内同构公式算期望（UTC 字段 → epoch），与实现互证
    local t = { day = 1, month = 1, year = 2030, hour = 0, min = 0, sec = 0 }
    local offset = os.time(os.date("!*t")) - os.time()
    local expected = (os.time(t) - offset) * 1000
    assert_eq("GMT epoch matches UTC interpretation", expected, epoch_ms)
    -- 差分性质：12 小时差 → 43200000 ms（时区无关）
    local p2 = Cookies.parseHttpDate("Fri, 01 Jan 2030 12:00:00 GMT")
    assert_eq("12h delta", 43200000, p2 - epoch_ms)
    return true
end
