...
function EzVenera:_startSettingsAutoflush()
    ...
    tick = function()
        ...
        -- L9（审查报告 §4）：30s→120s——闪存写频率降为 1/4，
        -- 崩溃丢失设置的最大窗口 2 分钟仍可接受（原始修复目标不变）
...
--- 周期排空 promise 微任务与到期定时器（H1 的 __ezv_timers）。
        ...
        sub_item_table_func = function()
            ...
            },
            {
                ...
                sub_item_table = {
                    ...
                    {
                        text = "从本地目录整包导入（manifest.json）",
                        callback = function()
                            guardCallback("整包导入本地源", function()
                                plugin:importLocalBundle()
                            end)
                        end,
                    },
                    {
                    ...
                    },
                    {
                        text = "批量删除源（多选）",
                        callback = function()
                            guardCallback("批量删除源", function()
                                plugin:getBrowser():showSourceMultiDelete()
                            end)
                        end,
                    },
                },
                ...
                callback = function()
                    guardCallback("引擎与状态", function()
                        ...
                        local lines = {
                            ...
                            -- M4（审查报告 §3）：TLS 校验状态对用户可见
                            "TLS 校验: 关（verify=none，与上游一致）"
                                .. "；公共 Wi-Fi 下有中间人风险",
                            "版本: M2（导航/面包屑/异步泵）",
                        }
                ...
                },
                {
                    ...
                    callback = function()
                        ...
                        if type(v) == "string" and v ~= "" then
                            -- M5（审查报告 §3）：on_ok 由各调用方提供，
                            -- 这里统一走守卫——格式化崩面不再击穿主循环
                            guardCallback(tostring(title), function()
                                on_ok(v)
                            end)
                        end
                    end,
...
--- 本地整包导入：一个目录里放 manifest.json（与官方 index.json 同 schema）
