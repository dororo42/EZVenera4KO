...
--- path 形如 "category" / "search.load" / "comic.loadInfo"。
...
function Browser:_awaitSource(key, path, argsJson)
    ...
    -- "comic.loadInfo" → "s?.comic?.loadInfo"：中间成员缺失时得 undefined
    -- 而非 TypeError（可选链 quickjs-ng 支持，源 JS 本身大量使用）
    local chain = ("s?." .. path:gsub("%.", "?."))
...
-- `parts` 是**分组**（包子 7 组、拷贝 2 组、再漫画 2 组），categories 为扁平
-- 字符串数组（不是 {label,target}）。旧代码直读 `category.categories` 恒为
-- nil → 主页只剩「搜索」一项。
...
function Browser:showSourceHome(key)
    ...
    elseif err and err:find("missing", 1, true) == nil then
        ...
        self.infoMessage(_("分类声明读取失败：") .. tostring(err))
    end
    local menu = Menu:new{
        title = key,
        item_table = item_table,
...
--- methodPath: "search.load" | "categoryComics.load"
...
function Browser:showResults(key, methodPath, argsTable, page, title)
    ...
    local res, err = self:_awaitSource(key, methodPath,
        "[" .. table.concat(parts, ",") .. "]")
    if not res then
        self.infoMessage(_("加载失败：") .. tostring(err) .. loginHint(err))
        return
    end
    local comics = res.comics or {}
    ...
    local menu = Menu:new{
        title = (title or key) .. (" · %d/%s"):format(page or 1,
            tostring(maxPage)),
        item_table = item_table,
        covers_fullscreen = true,
        is_borderless = true,
...
function Browser:showDetail(key, comic)
    ...
    local res, err = self:_awaitSource(key, "comic.loadInfo",
        '[' .. '"' .. jesc(comic.id) .. '"' .. ']')
    if not res then
        self.infoMessage(_("详情加载失败：") .. tostring(err) .. loginHint(err))
        return
    end
    local details = res
    ...
    UIManager:show(ConfirmBox:new{
        ...
        ok_callback = function()
            self:_guard(_("删除源"), function()
                local ok2 = self.sources and self.sources:remove(key)
                if ok2 then
                    self:invalidateSource(key)
                    self.infoMessage(_("已删除 ") .. tostring(key))
                else
                    self.infoMessage(_("删除失败"))
                end
            end)
        end,
    })
...
function Browser:showSourceMenu(key, name, version)
    ...
    local info, logged
    if jsKey then
        info = self.engine:sourceInfo(jsKey)
        logged = self:_srcdata():isLogged(jsKey)
    end
    local acc = info and info.account
    ...
    local site = acc and (acc.website or acc.register)
    if site then
    ...
    local menu
    menu = Menu:new{
        title = (name or key) .. "  v" .. tostring(version or "?"),
        item_table = rows,
        covers_fullscreen = true,
        is_borderless = true,
        is_popout = false,
