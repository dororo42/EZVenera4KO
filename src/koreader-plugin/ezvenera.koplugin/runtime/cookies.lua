...
function Cookies:headerFor(url)
    ...
    local parts = {}
    for _, c in ipairs(list) do
        -- L5（审查报告 §4）：剥离控制字符，堵 CRLF 头注入
        parts[#parts + 1] = (tostring(c.name):gsub("%c", ""))
            .. "=" .. (tostring(c.value):gsub("%c", ""))
    end
...
function Cookies.parseHttpDate(s)
    ...
    if d and MONTHS[mon:lower()] then
        -- L4（审查报告 §4）：Expires 是 GMT，os.time 却按本机时区解释——
        -- UTC+8 上会话 cookie 早死 8 小时。标准校正：减去当前 UTC 偏移。
        local t = { day = tonumber(d), month = MONTHS[mon:lower()],
                    year = tonumber(y), hour = tonumber(h),
                    min = tonumber(m), sec = tonumber(sec) }
        local epoch = os.time(t)
        local offset = os.time(os.date("!*t")) - os.time()
        return (epoch - offset) * 1000
    end
    return nil
end
