...
//!
//! 状态：可编译骨架（cargo check 通过；vendor/quickjs 缺失时 build.rs
//! 跳过 C 编译，仅暴露 TODO 桩——函数尚无行为，S2 实装）。
...
///
/// # Safety
/// rt 必须来自 ezvjs_new 返回值（Box::into_raw 契约），且不得重复释放；
/// 实装 S2 时 JS_FreeContext/JS_FreeRuntime 必须在 drop 前（逆序）调用。
#[no_mangle]
pub extern "C" fn ezvjs_destroy(rt: *mut EzVjsRuntime) {
    if rt.is_null() { return; }
    // TODO: JS_FreeContext, JS_FreeRuntime（先于 Box drop）
