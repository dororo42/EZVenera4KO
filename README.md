...
│   ├── 已添加的自定义索引      # 查看 / 删除
│   ├── 从本地文件添加源
│   ├── 整包导入本地源          # 目录内 manifest.json/index.json 一次装一批
│   ├── 批量删除源              # 多选删除
...
1. **代理**（源站在你这边不通时才需要）：工具 → EZVenera 漫画 → 网络代理 →
   编辑代理地址（例：`http://<IP>:7890`，文档用 TEST-NET 地址）→ 启用代理 → 测试连通。
2. **加源**：管理漫画源 → 浏览索引并安装 → 选一条 → 安装。
   有别人的索引或单个 `.js`：用「添加自定义源索引（index.json）」/「从 URL 添加源」。
   整包（一个目录 + `manifest.json`/`index.json`）用「整包导入本地源」。
   **版本护栏**：同 key 的源，包内版本更高或相同 → 覆盖更新；更低 → 默认跳过并单列报出，
   确认「仍然覆盖」才降级。每次覆盖前旧文件自动备份成 `<key>.js.bak-<版本>`，
   在「已安装源 → 该源 → 回退到旧版…」随时退回去。
...
| 覆盖前的旧版备份 | `<koreader>/ezvenera/sources/<key>.js.bak-<版本>` |
