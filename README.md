...
不依赖长按）。**当前主力设备：安卓平板**（实测华为 M2 / Android 6 / KOReader v2026.07.1）。
...
这两份就是 2026-09-23 真机（华为 M2 / Android 6）跑通 dlopen→桥→init.js→源 JS
...
make check        # = 语法 + 单测 + 无长按红线 + vendored 校验（CI 等价物）
```
