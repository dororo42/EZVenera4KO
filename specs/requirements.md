# EZVenera for KOReader — 需求规格（v1）

> 状态：G1 PASSED（2026-09-20）
> 溯源链：REQ → AC → DESIGN/ADR → TASK → CODE → TEST → VERIFY
> 证据分级：需求本身为用户输入与 G0 研究的规范化；每条 AC 必须可观测。

## 0. 产品定义

**EZVenera for KOReader**（`ezvenera.koplugin`）：在 KOReader（Lua）内运行 Venera/EZVenera JS 漫画源插件的移植层，首发目标设备为 **Kindle 4（非触屏）**，同时不破坏触屏设备可用性。
...
**AC**
- AC2.1 在 Kindle 4（或非触屏模拟路径）上仅用 D-pad+Press 可完成：开启代理→修改地址→保存→测试通过，全程无长按操作。
...
- AC2.4 单测：`foo`、`<IP>`（缺 scheme）、`http://`（缺 host:port）被拒绝；`http://<IP>:16492`（RFC 5737 文档地址）通过。
- AC2.5 真机（设备在 <LAN- subnet> 网段）：代理开启后能通过代理访问外网源并加载一页漫画（人工验收）。
...
| REQ | 内容 | 优先级 |
|---|---|---|
| R4.1 | 页图用 ImageViewer 图像列表模式 + 惰性下载（opdspse 蓝图），D-pad/PgFwd/PgBack 翻页 | P1 |
...
### R5 Kindle 4 / e-ink 操作适配
| REQ | 内容 | 优先级 |
|---|---|---|
| R5.1 | 全部菜单/对话框 D-pad 可达可操作；不依赖 hold/长按路径 | P0 |
| R5.2 | 文本输入提供两种路径：虚拟键盘（ScreenKB）+ 直接编辑设置文件指引 | P1 |
| R5.3 | 不假设 Amazon 服务可用；插件更新经用户显式触发 | P1 |

**AC**
- AC5.1 走查清单（菜单→代理→浏览→阅读→返回）全部动作仅用 Up/Down/Left/Right/Press/Back/Menu 完成（真机人工）。
- AC5.2 grep 检查插件代码无 `hold_input`/`hold_callback` 依赖关键路径（脚本化检查，随 CI/测试跑）。

### R6 JS 引擎
| REQ | 内容 | 优先级 |
|---|---|---|
| R6.1 | quickjs-ng 以 `arm-kindle5-linux-gnueabi`（softfp/glibc2.12）交叉编译为 `lib/libquickjs.so`，构建脚本入仓 | P0 |
...
| 真机清单 | AC1.1/AC2.1/AC2.5/AC4.1/AC4.2/AC5.1 人工走查（Kindle 4 + USBNet 调试） |
