...
> 接续 `ADDENDUM-M2-20260922.md`（M2=里程碑编号，非设备）——本轮把 R-D2v5 判处"死刑"的安卓引擎链路。本轮把 R-D2v5 判处"死刑"的安卓引擎链路
...
- 残留 benign：linker `readlink Permission denied`/`unable to get realpath`
  警告（该 Android 6 ROM 全局行为，不影响加载）

## 5. 引擎构建配方（构建机：/tmp/build_old_recipe.sh + build_nopatchelf.sh）
