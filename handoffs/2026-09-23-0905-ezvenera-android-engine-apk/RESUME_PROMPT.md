...
- **安卓引擎链路全通**（华为 M2 / Android 6 / arm64）：引擎 .so 零 patchelf 重编 → 注入 fdroid APK 重签安装 → dlopen/shim/init.js/源注册/navigation 全链稳定，用户真机实测零崩溃（2026-09-23 08:40 logcat 实证）。
...
2. 真机验证需网络（设备当前无外网；settings 残留代理 <DEV-HOST-LAN-IP>:<PROXY-PORT> 未验证）。
