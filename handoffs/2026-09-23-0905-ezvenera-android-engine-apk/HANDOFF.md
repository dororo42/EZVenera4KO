...
- [ ] 设备无外网（直连全超时）——搜索/分类数据流真机验证需网络恢复或代理（settings 里残留 `http://<DEV-HOST-LAN-IP>:<PROXY-PORT>` 代理配置，可用性未验证）
...
- **华为 force-stop/kill 不可靠**——重启 app 后必须验证新 PID + "loading libluajit-launcher" 日志。
- **构建机通道**：`ssh <BUILD-HOST-SSH-ALIAS>`（免密可用）；sudo 密码 <REDACTED> 已装 apksigner/zipalign；NDK r27c 在 `~/android-ndk`。
- **构建机上 heredoc 陷阱**：`$HOME` 会被 Windows 展开成 `C:Users...`——脚本一律写本地文件 scp 过去执行（build/apk-engine/ 内有全套现成脚本）。
- **K4 BusyBox grep 无 -E**；K4 SSH 需 pty（`ssh -tt -p 2222 <USER>@<KINDLE-LAN-IP>`，空密码）——K4 冻结中，恢复前不用管。
...
- 引擎重编（构建机）：`scp build\apk-engine\build_nopatchelf.sh <BUILD-HOST-SSH-ALIAS>:/tmp/ && ssh <BUILD-HOST-SSH-ALIAS> "bash /tmp/build_nopatchelf.sh"`（先跑 build_old_recipe.sh 产出 build/quickjs-android-old/）
...
**安全提醒**：定稿前如有 validate_handoff.py（agent-handoff-delivery skill 侧）可跑则跑；检测到密钥或质量分 <70 不得交付。本 handoff 不含任何凭据（构建机 sudo 密码<REDACTED> 6）。
