...
# C1（审查报告 §1，2026-09-25）：裸 `python` 在 Windows 上常是商店桩
# （exit 49 静默失败）——按 venv → python3 → python 顺序探测。
PY=""
for cand in "$ROOT/tools/.venv/Scripts/python.exe" "$ROOT/tools/.venv/bin/python"             "$(command -v python3)" "$(command -v python)"; do
    if [ -n "$cand" ] && [ -x "$cand" ] && "$cand" -c "import sys" 2>/dev/null; then
        PY="$cand"
        break
    fi
done
if [ -z "$PY" ]; then
    echo "[err] 未找到可用的 Python（已试 venv/python3/python）" >&2
    exit 1
fi
