...
if [ "$TARGET" = "host" ] || [ "$TARGET" = "x86_64" ]; then
    ...
    if [ ! -f "$BUILD/libquickjs.so" ]; then
        echo "[err] libquickjs.so not built (check cmake output above)" >&2
        exit 1
    fi
    cp "$BUILD/libquickjs.so" "$OUTDIR/libquickjs.so"
    ...
    if [ ! -f "$VENDOR/quickjs.h" ]; then
        echo "[err] vendor/quickjs not found. Run scripts/fetch-quickjs.sh first." 2>&1
        exit 1
    fi
